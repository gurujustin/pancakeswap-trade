...
const Farms: React.FC<FarmsProps> = (farmsProps) => {
  ...
  const farmsList = useCallback(
    (farmsToDisplay, removed: boolean) => {
      ...
      const WDEFI_PER_BLOCK = (new BigNumber(farmsLP.find((farm) => farm.pid === KIWI_POOL_PID)?.wdefiPerBlock || 0)).div(new BigNumber(10).pow(18))
      const farmsToDisplayWithAPY: FarmWithStakedValue[] = farmsToDisplay.filter((farm) => farm.pid !== 11).map((farm) => {
        if (!farm.tokenAmount || !farm.lpTotalInQuoteToken || !farm.lpTotalInQuoteToken) {
          return farm
        }
        const kiwiRewardPerBlock = WDEFI_PER_BLOCK.times(farm.poolWeight)
        ...
        } else if (farm.quoteTokenSymbol === QuoteToken.WDEFI) {
